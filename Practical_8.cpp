...
class Teaching_employee : public employee{
    ...
    bool search_teaching_employee(int &data)
    {
        if(data==search_id())
        return true;
        else
    ...
    void get_teaching_employee_data()
    {
        obj++;
    ...
    void put_teaching_employee_data()
    {
        put_employee_data();
...
class Non_teaching_employee : public employee{
    ...
    bool search_non_teaching_employee(int &data)
    {
        if(data==search_id())
        return true;
        else
...
int main()
{
    ...
    {
        temp[i].get_teaching_employee_data();
    }
    cout<<endl<<"-----------------------------------------"<<endl<<endl;
...
//OUTPUT
    ...
    for(int i=0;i<teachers;i++)
    {
        bool flag=temp[i].search_teaching_employee(*employee_id);
        if(flag==true)
        {
            temp[i].put_teaching_employee_data();
            flag_output=true;
            break;
        }
    }
    if(flag_output==false)
    {
        for(int i=0;i<non_teachers;i++)
        {
            bool flag=non_temp[i].search_non_teaching_employee(*employee_id);
            if(flag==true)
            {
